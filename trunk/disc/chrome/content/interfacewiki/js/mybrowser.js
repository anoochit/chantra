...
function Activate(aEvent)
{
  ...
  if (link instanceof HTMLAnchorElement) {
    ...
    if ((link.href.indexOf("http://",0)==0)||(link.href.indexOf("https://",0)==0)) {
      ...
      if (link.href.indexOf(".exe")>0 || link.href.indexOf(".pdf")>0 || link.href.indexOf(".ods")>0 || link.href.indexOf(".jar")>0 || link.href.indexOf(".xpi")>0 || link.href.indexOf(".webapp")>0 || link.href.indexOf(".doc")>0 || link.href.indexOf(".dmg")>0 || link.href.indexOf(".lch")>0 || link.href.indexOf(".odt")>0 || link.href.indexOf(".doc")>0 || link.href.indexOf(".ott")>0) {
        my_shell_execute(link.href);
        aEvent.preventDefault();
        aEvent.stopPropagation();
      } else
...
Components.classes['@mozilla.org/embedding/browser/nsWebBrowserPersist;1']
    ...
    var ret = dlmgr.addDownload ( -1, s_uri, t_uri, "Type the name of the program", null,
null, -1, null, persist);
  ...
  fp.init(window, "Download", nsIFilePicker.modeSave);
  var res = fp.show();
  if (res == nsIFilePicker.returnOK){
    dl(uri, 'file://'+fp.file.path);	
  }
}
