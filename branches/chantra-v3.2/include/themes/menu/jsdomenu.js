...
function popUpSubMenu(menuItemObj) { // Private method
  ...
  if (parentMenuObj.style.position == "fixed") {
    ...
    if (parentMenuObj.mode == "static" && !ie50) {
      ...
      */
	  //alert(y+"="+getTop(menuItemObj)+"-"+getPropIntVal(menuObj, btw)+"-"+getScrollTop());
     ...
     menuObj.style.left = (getSubMenuLeftPos(menuObj, x, menuItemObj.offsetWidth) + getScrollLeft()) + px;
	 //alert("ie5"+y+":"+menuItemObj.offsetHeight);
      menuObj.style.top = (getSubMenuTopPos(menuObj, y, menuItemObj.offsetHeight) + getScrollTop()) + px;
    }
  }
...
/*	if (navigator.appVersion.search("Windows 9") == -1) {
		document.all.music.src="resource:///disctree/include/themes/sounds/menu.wav";
	}*/
    var action = this.actionOnClick;
    if (action.indexOf("link:") == 0) {
      window.content.location.href = "resource:///disctree/th/"+action.substr(5);
    }
    else {
      if (action.indexOf("code:") == 0) {
        ...
        window.content.location.href = "resource:///disctree/th/"+action;
      }
    }
  }
  if (!e) {
...
function addMenuItem(menuItemObj) { // Public method
  if (menuItemObj.displayText == "-") {
    ...
    if (typeof(itemElm.innerHTML) != "undefined") {
      ...
      textElm.innerHTML = menuItemObj.displayText;
    }
    else {
      textElm = document.createTextNode(menuItemObj.displayText);
    }
...
function initjsDOMenu() { // Public method
  ...
  if (navigator.appVersion.search("Windows 9") != -1) {  
	alert("เบราวเซอร์ของคุณคือ "+navigator.userAgent+"\nChantra อาจไม่สามารถทำงานบนวินโดวส์รุ่นนี้ได้\nคุณสามารถติดตั้งโปรแกรมโดยตรงได้โดยเข้าไปที่โฟลเดอร์ program");
  }
}
...
if (typeof(autoHideTimeLimit) == "undefined") {
  var autoHideTimeLimit = 100; // Public field
}
...
var intervalId = null; // Private field
